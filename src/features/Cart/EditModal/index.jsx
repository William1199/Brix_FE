...
const EditModal = ({ visible, onClose, data = {}, callback, listData }) => {
  const theme = useTheme();
  const [quantity, setQuantity] = useState({ value: data.quantity });
  const [loading, setLoading] = useState(false);
  const [selectedType, setSelectedType] = useState(() => {
    const item = data.productType.find((x) => x.id === data.typeID);
    return {
      color: item.colorID,
      size: item.sizeID,
      other: item.otherID,
  ...
  const isButtonEnabled = useMemo(() => {
    ...
    switch (_case) {
      case PRODUCT_TYPE_ENUM["color-size"]: {
        const item = data.productType.find((x) => x.id === data.typeID);
        return selectedType.color && selectedType.size
          ? selectedType.color !== item.colorID ||
              selectedType.size !== item.sizeID ||
              quantity.value !== data.quantity
          : false;
      }
      case PRODUCT_TYPE_ENUM.none: {
        return quantity.value !== data.quantity;
      }
      default: {
        const item = data.productType.find((x) => x.id === data.typeID);
        return (
          ...
          (selectedType.color !== item.colorID ||
            selectedType.size !== item.sizeID ||
            selectedType.other !== item.otherID ||
            quantity.value !== data.quantity)
        );
      }
    }
  }, [selectedType, quantity, data]);
  ...
  const handleSubmit = async () => {
    ...
    const prevProduct =
      listData.find((x) => x.productID === data.productID) || {};
    if (prevProduct.typeID === selectedType) {
      const clone = [...listData];
      const index = clone.findIndex((x) => x.productID === data.productID);
      clone[index] = {
        ...prevProduct,
        quantity: prevProduct.quantity + quantity.value,
      };
      bodyData = clone;
    }
    setLoading(true);
    const updatedData = await CartServices.updateCart(bodyData);
    if (!!updatedData) {
      setLoading(false);
      Toast.show({
        type: "success",
              ...
              >
                ...
                >
                  <Pressable
                    ...
                    onPress={!isButtonEnabled ? () => {} : handleSubmit}
                  >
                    <Text
                      style={{
