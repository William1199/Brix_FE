...
const SavedPost = () => {
  ...
  const renderListEmpty = () => (
    ...
    >
      ...
      >
        Hiện chưa có bài đăng nào bạn được yêu thích
      </Text>
    </View>
  );
  ...
  useEffect(() => {
    // call api
    (async () => {
      const { message, data, pagination } = await axiosInstance.get(
        "savepost/getAllSave",
        {
          params: {
      ...
      if (+message === API_RESPONSE_CODE.success) {
        var tmp = data.map((item) => item.contractorPost[0]);
        setPosts((prev) => ({
